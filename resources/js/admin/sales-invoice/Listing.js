...
Vue.component('sales-invoice-listing', {
    ...
    mounted() {
        if ( this.$parent.form.hasOwnProperty( 'sales_invoice_lines' ) && this.$parent.form.sales_invoice_lines.constructor === Object ) {
            for ( let type in this.$parent.form.sales_invoice_lines ) {
                let lines = this.$parent.form.sales_invoice_lines[ type ];

                for ( let line of lines ) {
                    this.sales_invoice_lines.push( type + '-' + line );
                }
            }
        }
    },
