...
*/
    ...
    *, ::after, ::before {
    ...
    @if( $po->project->status === 'DRAFT_VERSION' )
    .page_break:before,
    .container:before{
        position: absolute;
        top: 400px;
...
<div class="container">
            ...
            <tr>
                <td width="70%">
                    @if ( $po->vendor )
                        ...
                        <p>
                            {{ $po->vendor->suite }}, {{ $po->vendor->billing_address_street }}
                            <br>{{ $po->vendor->billing_address_city }}, {{ $po->vendor->billing_address_country }}
                            <br>Tel: {{ $po->vendor->phone }}
                            <br>Fax: {{ $po->vendor->fax }}
                        </p>
                    @endif
                </td>
                <td width="30%">
    ...
    <table width="100%">
        ...
        @foreach ( $po->fields as $mainKey => $spec )
            @if( $specCount === 0 )
                <hr>
            @endif
            ...
            <table class="mt-4 mb-4" width="100%">
                <tr>
                    <td width="50%">
                        ...
                        <table width="100%">
                            @foreach( $po->fields[$mainKey]['materials'] as $key => $materials )
                                <tr>
                    ...
                    <td width="50%">
                        ...
                        <table width="100%">
                            @foreach( $po->fields[$mainKey]['specs'] as $key => $specification )
                                <tr>
                                    <td class="font-weight-bold">{{ $key }}:</td>
                                    <td>{{ $specification ?? '-' }}</td>
                                </tr>
    ...
    <table width="100%">
        <tr>
            <td width="33.333%">
                ...
                <p>{{ $po->materials_in_at && ! is_int( $po->materials_in_at ) ? $po->materials_in_at->format( 'Y/m/d') : '-' }}</p>
            </td>
            <td width="33.333%">
                <h5 class="font-weight-bolder">FOB Date</h5>
                <p>{{ $po->fob_at && ! is_int( $po->fob_at ) ? $po->fob_at->format( 'Y/m/d') : '-' }}</p>
            </td>
            <td width="33.333%">
                <h5 class="font-weight-bolder">Delivery Date</h5>
                <p>{{ $po->delivery_at && ! is_int( $po->delivery_at ) ? $po->delivery_at->format( 'Y/m/d') : '-' }}</p>
            </td>
        </tr>
    </table>
